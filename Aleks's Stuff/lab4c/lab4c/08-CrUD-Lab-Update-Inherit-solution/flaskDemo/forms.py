...
myChoices2 = [(row['pnumber'],row['pnumber']) for row in results2]
# change
results=list()
for row in ssns:
    rowDict=row._asdict()
    results.append(rowDict)
myChoices = [(row['ssn'],row['ssn']) for row in results]
...
#  One of many ways to use SelectField or QuerySelectField.  Lots of issues using those fields!!
    essn = SelectField("Employee SSN", choices=myChoices)  # myChoices defined at top
    pno = SelectField("Project", choices=myChoices2, coerce = int)
    submit = SubmitField('Assign Project')
# the regexp works, and even gives an error message
#    mgr_start=DateField("Manager's Start Date:  yyyy-mm-dd",validators=[Regexp(regex)])
#    mgr_start = DateField("Manager's Start Date")s
    def validateProject(self, essn, pno):
        project = Works_On.query.filter_by(essn=essn.data, pno=pno.data).first()
        if project:
            raise ValidationError('That project assignment already exists :(')
#    mgr_start=DateField("Manager's Start Date", format='%Y-%m-%d')
